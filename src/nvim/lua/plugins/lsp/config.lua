...
local function make_buffer_mappings(event)
    ...
    vim.keymap.set({ 'n', 'v' }, '<space>la', vim.lsp.buf.code_action,{buffer = event.buf, desc = 'LSP code action ', silent = false, noremap=true})
    vim.keymap.set({ 'n', 'v' }, '<space>lA', function() vim.lsp.buf.code_action({apply=true}) end,{buffer = event.buf, desc = 'LSP code action apply', silent = false, noremap=true})
